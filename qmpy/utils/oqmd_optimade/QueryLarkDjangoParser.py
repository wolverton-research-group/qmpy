...
class LarkParser:
    ...
    def parse(self, filter_):
        ...
        Examples::
            >>> print(LarkParser().parse("nsites>1 OR band_gap=3").pretty())
                filter
                   expression
                     expression_clause
                       expression_phrase
                         ...
                         comparison
                           property_first_comparison
                             ...
                             value_op_rhs
                               ...
                               value
                                 number	3
            >>> print(LarkParser().parse('chemical_formula_reduced=Al2O3').pretty())
                   ...
                   expression
                     expression_clause
                       expression_phrase
                         comparison
                           property_first_comparison
                             property	chemical_formula_reduced
                             value_op_rhs
                               =
                               value
                                 string	"Al2O3"
        ...
        try:
            ...
            error_message  = "Error while parsing the filter with optimade grammar version: "
            error_message += "v{}. ".format('.'.join([str(item) for item in self.version]))
...
class Lark2Django(Transformer):
    ...
    def __init__(self):
        ...
        self.warnings = []
        self.error_types = {'T1':"IgnoredProperty",
                           'T2':"InvalidLabel",
                           'T3':"NotSupported",
                           'T4':"GeneralWarning"
                          }
    ...
    def property_first_comparison(self,children):
        ...
        if children[1] is None:
            self.handle_error('T1',"Accompanying Operator or Value cannot be parsed",a.name)
            return
        ...
        if a.is_chem_form:
            ...
            if len(c_dict_lst)==1 and len(c_dict_lst[0].keys())==0:
                self.handle_error('T1',"Chemical formula {} cannot be parsed".format(b),a.name)
                return
            b = [" ".join(["%s%g" % (k, cd[k]) for k in sorted(cd.keys())]) for cd in c_dict_lst]
                
        if operation_fn in self.logic_functions:
            if not a.is_logic_operable:
                self.handle_error('T1',"Cannot be queried with operators <,>,<=,>=",a.name)
                return 
        try:
            ...
            self.handle_error('T1',"Unknown error while converting to Django Q",a.name)
    ...
    def length_op_rhs(self,children):
        if len(children)>2:
            self.handle_error('T3',"Not supported with >,<,=,etc. (Allowed format: property LENGTH value)",'LENGTH')
    ...
    def evaluate(self, tree):
        ...
        """
        error_message_500 = "Your query could not be transformed to a Django query filter ."
