...
class RESTfulTestCase(TestCase):
    ...
    def test_djangoQ_printer(self):
        ...
        assert truth_value == self.transform_q(
            'NOT stability>0 AND (_oqmd_band_gap>2 OR elements HAS ANY "Fe","Mn") OR (_oqmd_band_gap>2 AND (NOT generic="AB" OR _oqmd_spacegroup="Fm-3m"))',
            2,
        )
    ...
    def test_queries(self):
        ...
        assert truth_value == self.transform_q(
            "stability=0     AND band_gap>0.5 OR volume>1.4e+2", 0
        )
        assert truth_value == self.transform_q(
            "stability=0ANDband_gap>0.5ORvolume>1.4e+2", 0
        )
        assert truth_value == self.transform_q(
            "stability=0 AND band_gap>0.5 OR volume>1.4e+2", 0
        )
