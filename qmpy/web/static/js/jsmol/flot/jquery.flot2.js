 ...
 */
	...
	/* jQuery Flot methods added by Bob Hanson for dynamic data modification:
...
(function($) {
    function Plot(target_, data_, options_) {
        ...
        this.getOptions = function() { return options; };  //[[[BH]]]
        this.getTarget = function() { return target; }; //[[[BH]]]
        this.setData = function(data) { series = data; };  //[[[BH]]]
        this.setAxes = function(data) { axes = data; };  //[[[BH]]]
        this.setOptions = function(data) { options = data; };  //[[[BH]]]
        this.setTarget = function(data) { target = data; }; //[[[BH]]]
...
//        alert(xaxis.majorTicks)
        ...
        function processData() {
            ...
            for (var i = 0; i < series.length; ++i) {
                ...
                if (s.bars.show) {
                    mindelta = s.bars.align == "left" ? 0 : -s.bars.barWidth/2;
                    maxdelta = mindelta + s.bars.barWidth;
                }
        ...
        function setTicks(axis, axisOptions) {
            ...
            axis.majorTicks = axisOptions.majorTicks || null //[[[BH]]]
...
//[[[BH ---
            ...
            // draw major tick grid lines darker

            if (axes.xaxis.majorTicks || axes.yaxis.majorTicks) {
              ctx.stroke()
              ctx.beginPath()
            }
            
            if (axes.xaxis.majorTicks) {
              axis = axes.xaxis;
              ctx.strokeStyle = options.grid.color;
              
              var v, axis = axes.xaxis;
              for (i = 0; i < axis.majorTicks.length; ++i) {
                  v = axis.majorTicks[i];
                  if (v <= axis.min || v >= axes.xaxis.max)
                      continue;   // skip those lying on the axes
                  ctx.moveTo(Math.floor(axis.p2c(v)) + ctx.lineWidth/2, 0);
...
// --- BH]]]           
        ...
        function fixColors(A) {
        	for (var i = 0; i < A.length;i++)A[i] = parseColor(A[i])
	}
	
        function drawSeriesPoints(series) {
            function plotPoints(data, radius, fill, axisx, axisy, cols) {
                if (cols)fixColors(cols)
                for (var i = 0; i < data.length; ++i) {
                    if (data[i] == null)
                        continue;
                    ...
                    if (cols) ctx.strokeStyle = "" + cols[i]
                    if (cols) ctx.fillStyle = "" + cols[i]
                    ctx.beginPath();
            ...
            function plotPointShadows(data, offset, radius, axisx, axisy, colors) {
                if (colors)fixColors(colors)
                for (var i = 0; i < data.length; ++i) {
                    if (data[i] == null)
                        continue;
                    ...
                    ctx.beginPath();
                    if (colors)  ctx.fillStype = colors[i]
            ...
            plotPoints(series.data, series.points.radius, series.points.fill,
                       series.xaxis, series.yaxis,options.points.colors);
            ctx.restore();
        }

        function drawBar(color, x, y, barLeft, barRight, offset, fill, axisx, axisy, c) {
           if (!c)return
        ...
        function drawSeriesBars(series) {
            ...
            function plotBars(is3D, data, barLeft, barRight, offset, fill, axisx, axisy, colors) {
                for (var i = 0; i < data.length; i++) {
                    var d = data[i]
                    if (d == null)
                        continue;
                    if (is3D) {
                      var n = d[1][1].length
                      var dx = (barRight - barLeft) / n
                      var bx1 = barLeft / n
                      var bx2 = bx1 + dx
                      for (var j = 0; j < n; j++) {
                        var x = d[0] + barLeft + (j + 0.5) * dx
                        var y = d[1][1][j]
                        drawBar(colors ? colors[i] : 0, x, y, bx1, bx2, offset, fill, axisx, axisy, ctx);
                      }
                    } else {
                      drawBar(colors ? colors[i] : 0, d[0], d[1], barLeft, barRight, offset, fill, axisx, axisy, ctx);
                    }
                }
            }
            ...
            plotBars(series.bars.is3D, series.data, barLeft, barLeft + series.bars.barWidth, 0, series.bars.fill, series.xaxis, series.yaxis, series.bars.colors);
            ctx.restore();
        }
        ...
        function findNearbyItem(mouseX, mouseY) {
            ...
            // here we are allowing for hovering over bands based on x or y position only //[[[BH]]]
            var mode = (options.selection.hoverMode == "x" ? 0 : options.selection.hoverMode == "y" ? 1 : 2)  //[[[BH]]]
            // faster processing if we just track i and j  //[[[BH]]]]
            var thisi = -1, thisj = -1, thisbarpt;  //[[[BH]]]]
            ...
            for (var i = 0; i < series.length; ++i) {
                ...
                var data = s.data,
                    ...
                    checkpoint = !(checkbar && !(s.lines.show || s.points.show)),
                ...
                for (var j = 0; j < data.length; ++j) {
                    ...
                    var x = data[j][0], y = data[j][1];
                    if (checkbar && !foundPoint) {
                    if (s.bars.is3D)y = y[0]                    
                        // For a bar graph, the cursor must be inside the bar
                        // and no other point can be nearby
                        if (mx >= x + barLeft && mx <= x + barRight 
                          && (mode == 0 || my >= Math.min(0, y) && my <= Math.max(0, y))
                          ) {
                              thisi = i; //[[[BH]]]]
                              thisj = j; //[[[BH]]]]
                              thisbarpt = (s.bars.is3D ? (mx - (x + barLeft))/(barRight - barLeft) : 0)
                        }
                    }
                    ...
                    // check bounding box first
                      if (mode != 1 && (x - mx > maxx || x - mx < -maxx)    //[[[BH]]]
                         || mode != 0 && (y - my > maxy || y - my < -maxy)) //[[[BH]]]
                         continue;
                         
                        // We have to calculate distances in pixels, not in
                        // data units, because the scale of the axes may be different
                        var dx = (mode == 1 ? 0 : Math.abs(axisx.p2c(x) - mouseX)),
                            dy = (mode == 0 ? 0 : Math.abs(axisy.p2c(y) - mouseY)),
                            dist = dx * dx + dy * dy;
            ...
            return (thisi <0 ? null : result(thisi, thisj, thisbarpt)); //[[[BH]]]]
        }
        ...
        function onMouseMove(ev) {
            ...
            if (options.grid.hoverable && !hoverTimeout) {
                hoverTimeout = setTimeout(onHover, hoverDelay || options.grid.hoverDelayDefault);  //[[[BH]]]]
            }
	    ...
	    var pos = { pageX: event.pageX, pageY: event.pageY, canvasX: canvasX, canvasY: canvasY }; //[[BH]] added screenX and screenY 
            if (axes.xaxis.used)
            ...
            if (item) {
                if (eventname=="plothover" && hoverDelay == 0) {
                    // too soon
                    hoverDelay = hoverDelay || options.grid.hoverDelay || 100;  //[[[BH]]]]
                    setTimeout(onHover,hoverDelay);
                    return;
                }
                ...
                hoverDelay = options.grid.hoverDelay  //[[[BH]]]]
            }
        ...
        function drawPointHighlight(series, point) {
            ...
            octx.strokeStyle = parseColor(series.color).scale(1, 0.5, 0.5, 0.75).toString(); //[[bh]] somewhat more pronounced
...
//[[[BH
    ...
    $.replot = function(plotobject) {
        return new Plot(plotobject.getTarget(), plotobject.getData(), plotobject.getOptions());
    };
...
// BH]]]    
    ...
    function parseColor(str) {
        var result;
	if (typeof str == "object") return new Color(str[0], str[1], str[2]) //[[[bh]]]
