...
Clazz.declarePackage ("J.script");
Clazz.load (["J.api.JmolScriptManager", "J.util.JmolList"], "J.script.ScriptManager", ["java.lang.Boolean", "$.Thread", "J.api.Interface", "J.io.JmolBinary", "J.script.ScriptQueueThread", "J.util.Escape", "$.Logger", "$.SB", "$.TextFormat"], function () {
c$ = Clazz.decorateAsClass (function () {
this.viewer = null;
this.eval = null;
this.evalTemp = null;
...
Clazz.declarePackage ("J.script");
Clazz.load (["J.api.JmolScriptEvaluator"], "J.script.ScriptEvaluator", ["java.lang.Boolean", "$.Float", "$.NullPointerException", "$.Thread", "java.util.Hashtable", "J.api.Interface", "J.atomdata.RadiusData", "J.constant.EnumAnimationMode", "$.EnumPalette", "$.EnumStereoMode", "$.EnumStructure", "$.EnumVdw", "J.i18n.GT", "J.io.JmolBinary", "J.modelset.Atom", "$.Bond", "$.Group", "$.MeasurementData", "$.ModelCollection", "$.TickInfo", "J.script.FileLoadThread", "$.SV", "$.ScriptCompiler", "$.ScriptContext", "$.ScriptDelayThread", "$.ScriptException", "$.ScriptInterruption", "$.ScriptMathProcessor", "$.T", "J.util.ArrayUtil", "$.BS", "$.BSUtil", "$.C", "$.ColorEncoder", "$.ColorUtil", "$.Elements", "$.Escape", "$.GData", "$.JmolEdge", "$.JmolList", "$.Logger", "$.Matrix3f", "$.Matrix4f", "$.Measure", "$.P3", "$.P4", "$.Parser", "$.Point3fi", "$.Quaternion", "$.SB", "$.TextFormat", "$.V3", "J.viewer.ActionManager", "$.FileManager", "$.JC", "$.StateManager", "$.Viewer"], function () {
c$ = Clazz.decorateAsClass (function () {
...
break;
case 4:
fileName = this.optParameterAsString (1);
...
looping = true;
i = 3;
axis = (this.tokAt (3) == 2 ? "y" : this.optParameterAsString (i++).toLowerCase ());
...
s = J.util.TextFormat.simpleReplace (s, "10", "" + n);
break;
case 1611141175:
looping = true;
i = 3;
axis = this.optParameterAsString (i).toLowerCase ();
if (axis.length > 0) i++;
...
s = J.util.TextFormat.simpleReplace (s, "Y", axis);
s = "capture " + J.util.Escape.eS (fileName) + sfps + s + ";capture;";
this.script (0, null, s);
return;
}if (params != null) params =  new java.util.Hashtable ();
mode = 1073742032;
params =  new java.util.Hashtable ();
if (!looping) this.showString (J.i18n.GT._ ("Note: Enable looping using {0}", ["ANIMATION MODE LOOP"]));
this.showString (J.i18n.GT._ ("Animation delay based on: {0}", ["ANIMATION FPS " + fps]));
...
J.util.Logger.info (this.viewer.processWriteOrCapture (params));
}, $fz.isPrivate = true, $fz));
$_M(c$, "setCursorWait", 
function (TF) {
...
$_M(c$, "script", 
($fz = function (tok, filename, theScript) {
var loadCheck = true;
var isCheck = false;
var doStep = false;
var lineNumber = 0;
...
var text = "";
var applet = "";
var port = J.util.Parser.parseInt (this.optParameterAsString (1));
if (port == -2147483648) {
port = 0;
switch (this.slen) {
case 1:
...
$_M(c$, "getFlexFitList", 
function (bs1, bs2, smiles1) {
var mapSet = J.util.ArrayUtil.newInt2 (2);
this.getSmilesCorrelation (bs1, bs2, smiles1, null, null, null, null, true, false, mapSet, null);
...
var atoms = this.viewer.modelSet.atoms;
J.script.ScriptEvaluator.getTorsions (atoms, bondMap2, angles, 0);
J.script.ScriptEvaluator.getTorsions (atoms, bondMap1, angles, 1);
...
$_M(c$, "write", 
function (args) {
return this.getExtension ().write (args);
}, "~A");
$_M(c$, "getBitsetIdent", 
function (bs, label, tokenValue, useAtomMap, index, isExplicitlyAll) {
return this.getExtension ().getBitsetIdent (bs, label, tokenValue, useAtomMap, index, isExplicitlyAll);
}, "J.util.BS,~S,~O,~B,~N,~B");
$_M(c$, "setContactBitSets", 
function (bsA, bsB, localOnly, distance, rd, warnMultiModel) {
return this.getExtension ().setContactBitSets (bsA, bsB, localOnly, distance, rd, warnMultiModel);
...
Clazz.declarePackage ("J.script");
Clazz.load (null, "J.script.ScriptMathProcessor", ["java.lang.Float", "java.util.Arrays", "$.Date", "$.Hashtable", "J.api.Interface", "J.atomdata.RadiusData", "J.constant.EnumVdw", "J.modelset.Bond", "$.MeasurementData", "J.script.SV", "$.ScriptEvaluator", "$.T", "J.util.ArrayUtil", "$.AxisAngle4f", "$.BS", "$.BSUtil", "$.ColorEncoder", "$.ColorUtil", "$.Escape", "$.JmolList", "$.JmolMolecule", "$.Logger", "$.Matrix3f", "$.Matrix4f", "$.Measure", "$.P3", "$.P4", "$.Parser", "$.Point3fi", "$.Quaternion", "$.SB", "$.TextFormat", "$.V3"], function () {
c$ = Clazz.decorateAsClass (function () {
this.chk = false;
...
var data = this.eval.getFlexFitList (bs1, bs2, smiles1);
return (data == null ? this.addXStr ("") : this.addXAF (data));
}if (isIsomer) {
