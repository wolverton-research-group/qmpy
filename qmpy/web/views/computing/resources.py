...
def allocation_view(request, allocation_id):
    ...
    if request.method == "POST":
        ...
        users = [
            k.split("_")[1] for k in list(p.keys()) if "user_" in k and p.get(k) == "on"
        ]
        ...
        projects = [
            ...
            if "project_" in k and p.get(k) == "on"
        ]
...
def user_view(request, user_id):
    ...
    return render_to_response(
        "computing/user.html", get_globals(data), RequestContext(request)
    )
...
def projects_view(request):
    ...
    return render_to_response(
        "computing/projects.html", get_globals(data), RequestContext(request)
    )
...
def hosts_view(request):
    ...
    return render_to_response(
        "computing/hosts.html", get_globals(data), RequestContext(request)
    )
