...
def task_view(request, task_id):
    ...
    if request.method == "POST":
        ...
        task.save()
    data = {"task": task}
    return render_to_response("computing/task.html", data, RequestContext(request))
...
def job_view(request, job_id):
    ...
    data = {"job": job, "files": in_dir, "stdout": stdout}
    if request.method == "POST":
        job.collect()
        data.update(csrf(request))
    return render_to_response("computing/job.html", data, RequestContext(request))
...
def queue_view(request):
    ...
    recent_ = Calculation.objects.filter(label="static", converged=True).order_by(
        "-id"
    )[:25]

    recent_name_dict = dict([(str(c.entry.id), format_html(c.comp)) for c in recent_])
    recent_finished_dict = dict(
        [
            (str(c.entry.id), c.entry.task_set.order_by("-finished")[0].finished)
            for c in recent_
        ]
    )
    recent_project_dict = dict(
        [
            (
                str(c.entry.id),
                ", ".join(
                    p.name for p in c.entry.task_set.order_by("-finished")[0].projects
                ),
            )
            for c in recent_
        ]
    ...
    recent_ids = list(
        [
            x[0]
            for x in sorted(
                list(recent_finished_dict.items()), key=lambda x: x[1], reverse=True
            )
        ]
    )
    count = running.count()
    ...
    data.update(csrf(request))
    return render_to_response("computing/queue.html", data, RequestContext(request))


def online_view(request):
    data = {"running": 1, "upcoming": 1}
    if request.method == "POST":
        rf = request.FILES
        if "myfilesingle" in rf:
            ...
            folder = os.path.join(
                ...
                "libraries",
                "Online_submit",
                datetime.datetime.now().strftime("%h-%d-%Y-%H-%M-%S"),
            )
        ...
        if "subjobs" in rp:
            ...
            for poscar in poscars:
                ...
                try:
                    en = Entry.create(
                        poscar, projects=[proj], keywords=["Online_submit"]
                    )
                except:
                    dup_new.write(poscar + "\n")
                    continue
            ...
            dup_new.close()
            os.rename(
                os.path.join(folder, "sub_log_new"), os.path.join(folder, "sub_log")
            )
            os.rename(
