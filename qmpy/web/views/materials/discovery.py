...
def disco_view(request):
    ...
    # stable prototypes
    form_all = Formation.objects.filter(
        fit="standard", stability__lt=0.0, entry__meta_data__value="prototype"
    )
    ...
    form_not_icsd = form_all.exclude(
        composition__entry__meta_data__value="icsd"
    ).distinct()
    res_not_icsd = form_not_icsd.values_list(
        "composition", "entry__prototype__name", "delta_e"
    )
    res_not_icsd = [
        [format_comp(parse_comp(c)), proto, de] for c, proto, de in res_not_icsd
    ]
    ...
    for fe in form_icsd:
        if fe.entry.id != fe.entry.duplicate_of.id:
            continue
        proto_name = (
            fe.entry.prototype.name if fe.entry.prototype is not None else "None"
        )
    ...
    data = {"not_icsd": res_not_icsd, "icsd": res_icsd}
    #            'type3': result3}
    data.update(csrf(request))
