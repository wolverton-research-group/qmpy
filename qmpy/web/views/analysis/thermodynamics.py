...
def gclp_view(request):
    ...
    return render(request, "analysis/gclp.html", data)


def phase_diagram_view(request):
    data = {"search": "", "chem_pots": "", "stability": 0.25}
    if request.method == "POST":
        p = request.POST
        if p.get("search"):
            data["search"] = p["search"]
        ...
        if p["action"] == "submit":
            ...
            data["phase_data"] = pdata.phases
            ps = PhaseSpace(
                data["search"], mus=data["chem_pots"], data=pdata, load=None
            )
    ...
    return render(request, "analysis/phase_diagram.html", context=get_globals(data))
...
def chem_pot_view(request):
    ...
    return render(request, "analysis/chem_pots.html", context=data)
