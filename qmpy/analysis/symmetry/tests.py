...
class SpacegroupTestCase(TestCase):
    ...
    def test_rotations(self):
        ...
        self.assertTrue(np.allclose(rot.matrix, [[-1, -1, -1], [1, 0, -1], [1, -1, 1]]))

    def test_operations(self):
        op = Operation.get("x,y,-y")
        self.assertTrue(
            np.allclose(op.rotation.matrix, [[1, 0, 0], [0, 1, 0], [0, -1, 0]])
        )
        ...
        self.assertTrue(
            np.allclose(op.rotation.matrix, [[1, 0, 1], [0, 1, -1], [1, 1, 1]])
        )
        self.assertTrue(np.allclose(op.translation.vector, [0.5, 0.333333, 0.0]))
