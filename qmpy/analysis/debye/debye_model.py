...
def debye_E_zp(thetaD, natoms):
    ...
    """
    E_zp = 9 * natoms * BOLTZCONST * thetaD / 8.0
    return E_zp
...
def debye_E_vib(T, thetaD, natoms):
    ...
    """
    E_vib = 9 * thetaD / 8.0 + 3 * T * debye_func(thetaD / T)
...
def debye_C_V(T, thetaD, natoms):
    ...
    """
    C_V = 4 * debye_func(thetaD / T) - 3 * (thetaD / T) / (sp.exp(thetaD / T) - 1.0)
    ...
    for T in temp:
        ...
        C_V = sp.append(
            ...
            * (
                ...
                3 * (thetaD / T) / (sp.exp(thetaD / T) - 1.0)
            ),
        )
    F_vib = E_vib - T * S_vib
    # print E_vib
